package gaurav_garg;

class MaxNumberAssignment7
{
	void max(int num1,int num2)
	{
		if(num1 > num2)
		{
			System.out.println(num1 +" is bigger between " + num1 +" and " + num2);
		}else
		{
			System.out.println(num2 +" is bigger between " + num1 +" and " + num2);
		}
	}
	
	void min(int num1,int num2)
	{
		if(num1 < num2)
		{
			System.out.println(num1 +" is smaller between " + num1 +" and " + num2);
		}else
		{
			System.out.println(num2 +" is smaller between " + num1 +" and " + num2);
		}
	}
	
	public static void main(String...s)
	{
		MaxNumberAssignment7 maxnumberassignment7= new MaxNumberAssignment7();
		maxnumberassignment7.min(10,17);
		maxnumberassignment7.max(10,17);
	}
}